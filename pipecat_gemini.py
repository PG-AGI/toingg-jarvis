...
async def run_playwright(action: str, params: str = "{}"):
    ...
    try:
        ...
        resp = requests.post(
            f"{LAUNCHER_BASE}/schedule_action",
            json={
                "name": f"playwright: {action}",
                "actions": [{"type": action, **action_params}],
                "trigger": {"delay_seconds": 1},
            },
            timeout=10,
        )
        if resp.status_code in (200, 201):
            return f"Playwright action '{action}' scheduled"
...
TOOLS_SCHEMA = ToolsSchema.from_standard_tools(
    [
        {
            "type": "function",
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "action": {
                            ...
                            "enum": ["navigate", "click", "fill", "get_text", "screenshot", "open_url", "wait"],
                        },
                        "params": {
                            "type": "string",
...
class JarvisGeminiPipeline:
    ...
    def build(self):
        ...
        llm_settings = GeminiLiveLLMSettings(
        ...
        llm = GeminiLiveLLMService(settings=llm_settings)
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    if msg_type == "audio":
                        # Audio binary from browser
                        audio_b64 = data.get("data", "")
                        # Forward to pipeline via input audio frame
                        # (Pipecat handles the rest)
                        pass
